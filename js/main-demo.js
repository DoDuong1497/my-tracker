...
search.addEventListener('input', searchDescription);

function searchDescription(e) {
  const cloneTrackerData = [...trackerData];
  const searchValue = e.target.value.toLowerCase();
  const searchData = cloneTrackerData.filter((data) =>
    data.title.toLowerCase().includes(searchValue)
  );
  renderListIssueTracker(searchData);
}
